...
Fecha: 06/07/2025
Descripción: Programa que calcula la compra de cada cliente en una tienda de ropa */
#include <stdio.h>
#include <string.h>
...
int main() {
    ...
    int vendidos[8] = {0};  // Cantidad acumulada por cada producto (índice 0 a 7)
    ...
    while (continuarClientes == 's' || continuarClientes == 'S') {
        ...
        if (strcmp(usuario, usuarioCorrecto) != 0 || strcmp(clave, claveCorrecta) != 0) {
            ...
            continue;  // Vuelve a solicitar nuevo cliente
        }
        ...
        while (seguir == 's' || seguir == 'S') {
          ...
          switch (opcion) {   //Nota. Tipo de prenda
            ...
            if (cantidad > 5) {
                descuento = subtotal * 0.10;  // 10% de descuento volumen
            }
        ...
        float cupon = 0;
        printf("\nIngrese el valor de su cupón de descuento (0 si no tiene) -> ");
        scanf("%f", &cupon);
